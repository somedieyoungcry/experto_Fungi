...
def cargar_bs():
    lista = open("Sintomas/simt_maiz.txt", encoding="utf8")
...
def abrir_ventana2():
    ...
    bg = PhotoImage(file="Imagenes/maiz.png")
...
def abrir_ventana_dg():
    ventana3 = Toplevel()
    app = ventana2(ventana3)
    bg = PhotoImage(file="Imagenes/descripcion.png")
    label = Label(ventana3, image=bg)
    label.place(x=0, y=0, relwidth=1, relheight=1)

    unframe = Label(ventana3, bg='#AEE25A', text="DIAGNÓSTICO")
    unframe.config(font= ("Amatic SC bold", 34), fg="black")
 
    unframe.place(y=99, x=170)

    label_usu = Label(ventana3, text="SU CULTIVO SUFRE DE " + pb.get_disganostico() , font= ("Amatic SC bold", 30), fg="black")
    ...
    label_des = Label(ventana3, text=lectura_enfermedades, font= ("Nunito", 15), fg="black",  wraplength=700, anchor="e", justify=LEFT)
    ...
    def abrir_ventana_tratamiento():
        ...
        label_trata = Label(ventana3, text=lectura_enfermedades, font= ("Nunito bold", 15), fg="black",  wraplength=500, anchor="e", justify=LEFT)
    ...
    validar = Button(ventana3, text="Siguiente", command=abrir_ventana_tratamiento)
...
class DiagnosticoEnfermedad(KnowledgeEngine):
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma1=MATCH.s1), NOT(Fact(sintoma2=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma2=MATCH.s2), NOT(Fact(cz=W())))
    def sintoma_CZ(self, s2):
        if s2 == "si":
            global pregunta
            pregunta = "Su cultivo sufre de Crbon del Maiz"
            #print("entro")
            pb.set_diagnostico("CARBON DEL MAIZ")
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma3=MATCH.s3), NOT(Fact(sintoma4=W())), NOT(Fact(cz=MATCH.cz)))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma4=MATCH.s4), NOT(Fact(sintoma5=W())), NOT(Fact(cz=MATCH.cz)))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma5=MATCH.s5), NOT(Fact(sintoma6=W())), NOT(Fact(cz=MATCH.cz)))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma6=MATCH.s6), NOT(Fact(cz=MATCH.cz)), NOT(Fact(fm=W())))
    def sintoma_FM(self, s6):
        if s6 == "si":
            global pregunta
            pregunta = "Su cultivo sufre de Fusarium"
            #print("entro")
            pb.set_diagnostico("FUSARIUM")
            respuesta_ms = messagebox.askyesno(message="De a cuerdo a las preguntas respondidas tenemos un diagnóstico, ¿Desea continuar?", title="Continuar")
            if respuesta_ms == True:
               abrir_ventana_dg()
               #print("Hola mundo")
