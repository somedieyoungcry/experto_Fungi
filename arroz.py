...
def cargar_bs():
    lista = open("Sintomas/enferm_arroz.txt", encoding="utf8")
...
def abrir_ventana2():
    ...
    bg = PhotoImage(file="Imagenes/arroz.png")
...
class DiagnosticoEnfermedad(KnowledgeEngine):
    ...
    @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(sintoma1=W())), Fact(sintoma0=MATCH.s0), NOT(Fact(sintoma5=W())))
    def sintoma_1_y_5(self, s0):
        if s0 == "si":
            global pregunta
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma3=MATCH.s3), NOT(Fact(sintoma4=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma4=MATCH.s4), NOT(Fact(mp=W())))
    def sintoma_MP(self, s4):
        if s4 == "si":
            global pregunta
            pregunta = "Su cultivo sufre de Mancha Parda de Arroz"
            
            pb.set_diagnostico("MANCHA PARDA DE ARROZ")
            ...
            self.declare(Fact(mp="True"))
            #print("Su cultivo sufre de Mancha Roja")
            #KnowledgeEngine.reset()
        
        
    @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(sintoma5=W())), NOT(Fact(mp=MATCH.mp)))
    def sintoma_5(self):
        global pregunta
        pregunta=sintomas_once[5]
        abrir_ventana2()
    ...
    @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(sintoma6=W())), Fact(sintoma5=MATCH.s5), NOT(Fact(mp=MATCH.mp)))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma6=MATCH.s6), NOT(Fact(mp=MATCH.mp)), NOT(Fact(sintoma7=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma7=MATCH.s7), NOT(Fact(mp=MATCH.mp)), NOT(Fact(sintoma8=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma8=MATCH.s8), NOT(Fact(mp=MATCH.mp)), NOT(Fact(sintoma9=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma9=MATCH.s9), NOT(Fact(mp=MATCH.mp)), NOT(Fact(sintoma10=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma10=MATCH.s10), NOT(Fact(mp=MATCH.mp)), NOT(Fact(sintoma11=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma11=MATCH.s11), NOT(Fact(mp=MATCH.mp)), NOT(Fact(sintoma12=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma12=MATCH.s12), NOT(Fact(mp=MATCH.mp)), NOT(Fact(sintoma13=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma13=MATCH.s13), NOT(Fact(mr=MATCH.mr)), NOT(Fact(pa=MATCH.pa)))
    def sintoma_PA(self,s13):
        if s13 == "si":
            pb.set_diagnostico("PYRICULARIA DE ARROZ")
            respuesta_ms = messagebox.askyesno(message="De a cuerdo a las preguntas respondidas tenemos un diagnóstico, ¿Desea continuar?", title="Continuar")
            if respuesta_ms == True:
               abrir_ventana_dg()
               #print("Hola mundo")
            ...
            self.declare(Fact(pa="Su cultivo sufre de Pyricularia de Arroz"))
