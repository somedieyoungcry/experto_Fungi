...
def cargar_bs():
    lista = open("Sintomas/enferm_berenjena.txt", encoding="utf8")
...
def abrir_ventana2():
    ...
    bg = PhotoImage(file="Imagenes/berenjena.png")
...
class DiagnosticoEnfermedad(KnowledgeEngine):
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma3=MATCH.s3), NOT(Fact(ac=W())))
    def sintoma_AC(self, s3):
        if s3 == "si":
            global pregunta
            pregunta = "Su cultivo sufre de Mancha Purpura"
            
            pb.set_diagnostico("ANTRACNOSIS")
    ...
    @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(sintoma4=W())), NOT(Fact(ac=MATCH.ac)))
    def sintoma_4(self):
        global pregunta
        pregunta=sintomas_once[4]
        abrir_ventana2()
    ...
    @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(sintoma5=W())), Fact(sintoma4=MATCH.s4), NOT(Fact(ac=MATCH.ac)))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(sintoma6=W())), Fact(sintoma5=MATCH.s5), NOT(Fact(ac=MATCH.ac)))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma6=MATCH.s6), NOT(Fact(mc=MATCH.mc)), NOT(Fact(sintoma7=W())))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma7=MATCH.s7), NOT(Fact(mc=MATCH.mc)), NOT(Fact(pf=MATCH.pf)))
    def sintoma_PF(self,s7):
        if s7 == "si":
            pb.set_diagnostico("PODREDUMBRE DE FRUTOS")
            respuesta_ms = messagebox.askyesno(message="De a cuerdo a las preguntas respondidas tenemos un diagnóstico, ¿Desea continuar?", title="Continuar")
            if respuesta_ms == True:
               abrir_ventana_dg()
               #print("Hola mundo")
            ...
            self.declare(Fact(pf="Su cultivo sufre de Podredumbre de Frutos"))
