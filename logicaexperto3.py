...
class DiagnosticoEnfermedad(KnowledgeEngine):
    ...
    def inicial(self):
        ...
        diccionario = {"sintoma0": [sintomas_once[1], sintomas_once[4]], "sintoma1": [
            sintomas_once[2]], "sintoma2": [sintomas_once[3]]}
        diccionario2 = {"sintoma0": [
            {"sintoma1": sintomas_once[1]}, {"sintoma4": sintomas_once[4]}]}
                ...
                for i in range(len(diccionario2[clave])):
                    for clave1, valor1 in diccionario2[clave][i].items():
                        ...
                        if len(sintom) > 1:
                            ...
                            print(sintoma1)
                            @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(sintoma364=W())))
                            def sintoma_364(self):
                                print("Hola mundo")
                                

            """
                    Se tiene que realizar una lista que me guarde las dos claves, "sintoma1" y "sintoma0", despues de ello crear la regla con los valores del item en ese momento arreglo[0]=
                """
            #Server = {}
