...
def cargar_bs():
    lista = open("Sintomas/simt_garbanzo.txt", encoding="utf8")
...
def abrir_ventana2():
    ...
    bg = PhotoImage(file="Imagenes/garbanzo.png")
...
class DiagnosticoEnfermedad(KnowledgeEngine):
    ...
    @Rule(Fact(action='encontrar_enfermedad'), NOT(Fact(rg=W())), Fact(sintoma0=MATCH.s0), NOT(Fact(sintoma1=W())))
    def sintoma_RG_y_1(self, s0):
        if s0 == "si":
            global pregunta
            pregunta = "Su cultivo sufre de Roya del Garbanzo"
            
            pb.set_diagnostico("ROYA DEL GARBANZO")
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma1=MATCH.s1), NOT(Fact(rg=MATCH.rg)), NOT(Fact(sintoma2=W())), NOT(Fact(sintoma3=W())))
    def sintoma2_y_3(self, s1):
        if s1 == "si":
            global pregunta
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma2=MATCH.s2), NOT(Fact(rg=MATCH.rg)), NOT(Fact(sm=W())))
    def sintoma_SM(self, s2):
        if s2 == "si":
            global pregunta
            pregunta = "Su cultivo sufre de Sclerotium"
            
            pb.set_diagnostico("SCLEROTIUM")
            respuesta_ms = messagebox.askyesno(message="De a cuerdo a las preguntas respondidas tenemos un diagnóstico, ¿Desea continuar?", title="Continuar")
            if respuesta_ms == True:
               abrir_ventana_dg()
               #print("Hola mundo")
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma3=MATCH.s3), NOT(Fact(sintoma4=W())), NOT(Fact(rg=MATCH.rg)), NOT(Fact(sm=MATCH.sm)))
    ...
    @Rule(Fact(action='encontrar_enfermedad'), Fact(sintoma4=MATCH.s4), NOT(Fact(fs=W())), NOT(Fact(rg=MATCH.rg)), NOT(Fact(sm=MATCH.sm)))
    def sintoma_FS(self, s4):
        if s4 == "si":
            global pregunta
            pregunta = "Su cultivo sufre de Fusariosis"
            #print("entro")
            pb.set_diagnostico("FUSARIOSIS")
            respuesta_ms = messagebox.askyesno(message="De a cuerdo a las preguntas respondidas tenemos un diagnóstico, ¿Desea continuar?", title="Continuar")
            if respuesta_ms == True:
               abrir_ventana_dg()
               #print("Hola mundo")
